  ...
  module DBApi
    ...
    class Statement < DB::Statement
      ...
      def initialize(connection : DBApi::Connection, @sql : String)
        ...
        @cass_statement = LibCass.cass_statement_new(@sql, 0)
      end

      def do_close
        LibCass.cass_statement_free(@cass_statement)
      end

      protected def perform_query(args : Enumerable) : ResultSet
        rebind_params(args)
        cass_result_future = LibCass.cass_session_execute(@session,
                                                          @cass_statement)
      ...
      private def rebind_params(args : Enumerable)
        LibCass.cass_statement_reset_parameters(@cass_statement, args.size)
